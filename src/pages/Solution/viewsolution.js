    ...
    if (viewSolution.solutionList.length > 1) {
      ...
      viewSolution.solutionList.forEach((solution, index) => {
        if (solution.id === submissionId) {
          curIndex = index;
          listDiv.push(<i className="solution-dot solution-dot-active" data-aa={solution.id}></i>);
          return;
        }
        ...
        listDiv.push(<i onClick={gotoCurSolution} className="solution-dot" data-aa={solution.id}></i>);
      });
    }
